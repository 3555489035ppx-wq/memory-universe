...
- 记忆数据 P0 不接在线 AI、远程图片、远程字体或 CDN；音乐层只读取随网站部署的同源系统音乐，或当前浏览器中的用户上传文件。
- `src/features/music` 负责系统音乐库、浏览器上传和播放队列；系统音频从 `public/music/` 读取，上传文件只保存在当前页面会话。音乐不参与情绪分析，也不改变宇宙视觉规则。
- 音乐文件不会上传到服务器，也不保存账号、Cookie 或第三方平台会话；导出时只接受仍可读取的系统文件或用户上传文件。
- 系统音乐库由 `scripts/generate-music-library.mjs` 扫描生成，未知音频会被标记为“待修改”，不会被伪装成已识别歌曲。
